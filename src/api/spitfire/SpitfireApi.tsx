...
export interface ISpitfireApi {
  ...
  cloneNote(noteId: string, name: string): void
...
export default class SpitfireApi extends React.Component<any, any>  implements ISpitfireApi {
  ...
  public componentWillReceiveProps(nextProps) {
    ...
    if (config && ! isEqual(config, this.config)) {
      this.config = config
      this.webSocketClient = new ReconnectingWebSocket('wss://spitfire.radiusiot.com/spitfire/ws')
    ...
    public newRestClient(username: string) {
        this.restClient = new RestClient({
            name: 'SpitfireApi',
            url: 'https://spitfire.radiusiot.com',
            path: '/spitfire/api/',
            username: username,
...
// ----------------------------------------------------------------------------
  ...
  public cloneNote(noteId: string, name: string): void {
    this.sendWebSocketMessage(JSON.stringify(this.CLONE_NOTE(noteId, name)))
  }
...
// ----------------------------------------------------------------------------
  ...
  private CLONE_NOTE(noteId: string, name: string) {
    return {
      'op':	'NEW_NOTE',
      'principal': this.principalValue(),
      'ticket':	this.ticketValue(),
